...
def wrap(f, derivatives_args=[], derivatives_kwargs={}):
    ...
    def f_with_affine_output(*args, **kwargs):
        ...
        names_w_uncert = set(key for (key, value) in kwargs.items()
                             if isinstance(value, AffineScalarFunc))
        ...
        for expr in itertools.chain(
            ...
            iter(kwargs_uncert_values.values())):  # From kwargs
        ...
        # ... on args:
        for (pos, f_derivative) in derivatives_num_args.items():
            for (var, arg_derivative) in args[pos].derivatives.items():
                derivatives_wrt_vars[var] += f_derivative * arg_derivative
        # ... on kwargs:
        for (name, f_derivative) in derivatives_num_kwargs.items():
            for (var, arg_derivative) in (iter(kwargs_uncert_values[name]
                                          .derivatives.items())):
                derivatives_wrt_vars[var] += f_derivative * arg_derivative
