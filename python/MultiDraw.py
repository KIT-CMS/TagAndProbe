...
def MultiDraw(self, Formulae, Compiled=False):
    ...
    for i, origFormula in enumerate(Formulae):
        ...
        if formula[-1] == ",":
            ...
            if pos_open_y == -1 or pos_close_y == -1 or pos_open_y > pos_close_y:
                raise RuntimeError("You bus")
            bin_args_y = GetBinningArgs(
                formula[pos_open_y + 1 : pos_close_y], var_binned_y
            )
        ...
        if not is_2d:
            ...
            hist = ROOT.TH2F(
                origFormula + ":" + weight, origFormula, *(bin_args_x + bin_args_y)
            )
    ...
    if Compiled:
        ...
        print("Compiled selector: %s" % fname)
        ...
        for line in fileinput.input("%s.C" % fname, inplace=1):
            if line.startswith("   return kTRUE;"):
                ...
                for i, f in enumerate(formulaeStr):
                    ...
                    print(
                        "   if (weight_value) static_cast<TH1D*>(hists->UncheckedAt(%i))->Fill(%s, weight_value);"
                        % (i, f)
                    )
                    # print '  std::cout << "%i %s:" << %s << "\\n";' % (i, weightsStr[i], weightsStr[i])
                print("   if (entry % 50000 == 0) {")
                print(
                    '      std::cout << "Done " << (double(entry) / (double(fChain->GetEntries())) * 100.0) << "%    \\r";'
                )
