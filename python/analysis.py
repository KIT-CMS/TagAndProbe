...
class Sel(object):
    ...
    def __call__(self, **kwargs):
        res = self.terms.copy()
        res.update(kwargs)
        return " * ".join(["(" + x + ")" for x in list(res.values())])
...
class Shape(object):
    ...
    def _Int(self):
        if self._hist.GetDimension() == 1:
            ...
            return self._hist.Integral(
                0, self._hist.GetNbinsX() + 1, 0, self._hist.GetNbinsY() + 1
            )
    ...
    def _IntErr(self):
        ...
        if self._hist.GetDimension() == 1:
            ...
            val = self._hist.IntegralAndError(
                0, self._hist.GetNbinsX() + 1, 0, self._hist.GetNbinsY() + 1, err
            )
    ...
    def Print(self):
        ...
        print(
            (
                "rate={:.3f}, entries={:g}, sum={:g}".format(
                    self.rate, self.hist.GetEntries(), self.hist.GetSumOfWeights()
                )
            )
        )
...
class SummedNode(ListNode):
    ...
    def RunSelf(self):
        self.shape = sum([node.shape for node in list(self.nodes.values())])
...
class HttWQCDCombinedNode(BaseNode):
    ...
    def RunSelf(self):
        ...
        if self.strategy == "simultaneous":
            self.qcd_hmt_ss = (
                self.data_hmt_ss.shape - self.sub_hmt_ss.shape - self.w_hmt_ss
            )
    ...
    def Objects(self):
        return {
            ...
            self.OutputPrefix(self.data_lmt_ss)
            + "/"
            + self.name_qcd: self.qcd_lmt_ss.hist,
            self.OutputPrefix(self.data_hmt_os) + "/" + self.name_w: self.w_hmt_os.hist,
            self.OutputPrefix(self.data_hmt_os)
            + "/"
            + self.name_qcd: self.qcd_hmt_os.hist,
            self.OutputPrefix(self.data_hmt_ss) + "/" + self.name_w: self.w_hmt_ss.hist,
            self.OutputPrefix(self.data_hmt_ss)
            + "/"
            + self.name_qcd: self.qcd_hmt_ss.hist,
        }
...
class Analysis(object):
    ...
    def BasicFactory(
        self, name, sample=None, var="", sel="", factors=[], scaleToLumi=True
    ):
        if sample is None:
            sample = name
        if scaleToLumi:
    ...
    def SummedFactory(
        self, name, samples, var="", sel="", factors=[], scaleToLumi=True
    ):
        res = SummedNode(name)
        for sa in samples:
