...
def main(
    ...
    if model_comparison:
        sig_models = ["DoubleVPartcorr", "DoubleVCorr", "DoubleVUncorr", "BWDoubleCBConvCorr"]
        bkg_models = ["CMSShape", "Exponential"]
        logger.info("Modellvergleich aktiviert - teste alle Kombinationen")
    else:
        sig_models = [sig_model]
        bkg_models = [bkg_model]
        logger.info(f"Feste Modell-Kombination: {sig_model} + {bkg_model}")
    ...
    if model_comparison:
        ...
        h2 = ROOT.TH2D("chi2_map", "Chi2 Map; p_{T} bin; #eta bin",
                        len(pt_bins)-1, array.array('d', pt_bins),
                        len(eta_bins)-1, array.array('d', eta_bins))

        hmod = ROOT.TH2D("accepted_model", "Accepted signal model per bin; p_{T} bin; #eta bin", 
                         len(pt_bins)-1, array.array('d', pt_bins), 
                         len(eta_bins)-1, array.array('d', eta_bins))
        
        hmod2 = ROOT.TH2D("accepted_model2", "Accepted background model per bin; p_{T} bin; #eta bin", 
                         len(pt_bins)-1, array.array('d', pt_bins), 
                         len(eta_bins)-1, array.array('d', eta_bins))

        chi2s = [float('inf')] * (hist.GetNbinsX() * hist.GetNbinsY())
        best_comb = [[0] * (hist.GetNbinsX() * hist.GetNbinsY()) for _ in range(2)]
        effs = [[[0.0 for _ in range(hist.GetNbinsX() * hist.GetNbinsY())] 
    ...
    for run_no, current_sig_model in enumerate(sig_models):
        for run_no2, current_bkg_model in enumerate(bkg_models):
            
            if model_comparison:
                logger.info(f"Teste Kombination: {current_sig_model} + {current_bkg_model}")
            
            wsp = infile.Get("wsp_" + name)
            if not wsp:
                raise RuntimeError(f"Workspace wsp_{name} nicht gefunden")

            pdf_args = []
            nparams = 1
            
            if current_sig_model == "DoubleVCorr":
                nparams = 6
                pdf_args.extend([
            ...
            elif current_sig_model == "DoubleVUncorr":
                nparams = 12
                pdf_args.extend([
            ...
            elif current_sig_model == "DoubleVPartcorr":
                nparams = 6
                pdf_args.extend([
            ...
            elif current_sig_model == "BWDoubleCBConvCorr":
                nparams = 15
                pdf_args.extend([
                ...
                raise RuntimeError("Chosen --sig-model %s not supported" % current_sig_model)

            if current_bkg_model == "Exponential":
                nparams += 2
                pdf_args.extend([
                    "Exponential::backgroundPass(m_vis, lp[-0.1,-1,0])",
                    "Exponential::backgroundFail(m_vis, lf[-0.1,-1,0])",
                ])
            elif current_bkg_model == "CMSShape":
                nparams += 4
                pdf_args.extend([
            ...
            elif current_bkg_model == "Chebychev":
                nparams += 6
                pdf_args.extend([
                ...
                raise RuntimeError("Chosen --bkg-model %s not supported" % current_bkg_model)
            ...
            for i, b in enumerate(bins):
                ...
                make_plots = (not model_comparison) or (run_no == 0 and run_no2 == 0)
                ...
                if model_comparison:
                    ...
                    splitData.At(1).plotOn(xframe, ROOT.RooFit.DataError(ROOT.RooAbsData.Poisson))
                    wsp.pdf("passing").plotOn(xframe, ROOT.RooFit.Slice(wsp.cat("cat"), "pass"))
                    
                    current_chi2 = xframe.chiSquare("AllPass", "DataPass", nparams)
                    effs[run_no][run_no2][i] = eff.getVal()
                    
                    if current_chi2 < chi2s[i]:
                        chi2s[i] = current_chi2
                        best_comb[0][i] = run_no
                        best_comb[1][i] = run_no2
                        
                        hist.SetBinContent(b[0], b[1], eff.getVal())
                        hist.SetBinError(b[0], b[1], eff.getError())
                        
                        h2.SetBinContent(b[0], b[1], current_chi2)
    ...
    if passfail_hist.lower() == "yes" and (not model_comparison or len(chi2_pass_array) > 0):
        ...
        data_type = os.path.basename(plot_dir)
    ...
    if model_comparison:
        ...
        hmod.Write()
        hmod2.Write()
        logger.info("Modellvergleichs-Daten gespeichert")
    ...
    if model_comparison:
        logger.info("Modellvergleich abgeschlossen")
    else:
        logger.info("Standard-Fit abgeschlossen")
