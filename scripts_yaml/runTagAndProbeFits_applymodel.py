#!/bin/env python
import subprocess
import fitTagAndProbe_script_applymodel
...
parser.add_argument("--yaml-base-path", required=True, help="Path to YAML files with best model configurations")
...
if args.fit:
    ...
    for expression in expression_list:
        p = Process(target=fitTagAndProbe_script_applymodel.main, args=tuple(expression))
        procs.append(p)
        p.start()
    for p in procs:
        p.join()
