...
def main():
    ...
    # DEBUG: Zeige den kompletten String
    print(f"DEBUG: Full json-name argument = '{args.json_name}'")
    print(f"DEBUG: Type = {type(args.json_name)}")
    
    base = os.path.basename(args.json_name)
    print(f"DEBUG: Base filename = '{base}'")
    
    prefix = base.split("_")
    print(f"DEBUG: After split = {prefix}")
    print(f"DEBUG: Number of elements = {len(prefix)}")
