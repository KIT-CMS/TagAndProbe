...
def main(
    ...
    for run_no, sig_model in enumerate(sig_models):
        if (sig_model != "DoubleVUncorr") and ENABLE_TEST_BIN:
            continue
        for run_no2, bkg_model in enumerate(bkg_models): # remove
            if (bkg_model != "CMSShape") and ENABLE_TEST_BIN:
                continue
            
            if (sig_model, bkg_model) not in memo:
                memo[(sig_model, bkg_model)] = deepcopy(wsp)
            
            _wsp = memo[(sig_model, bkg_model)]

            wsp = wsp_template.Clone(f"wsp_{sig_model}_{bkg_model}")       
            ...
            # chi2s = [float('inf')] * n_bins
            # best_comb = [[0]*n_bins, [0]*n_bins]
            # man_calc = [0]*n_bins
            # effs_sig = [[0]*n_bins for _ in range(len(sig_models))]
            # effs_bkg = [[0]*n_bins for _ in range(len(bkg_models))]
            # effs = [[[0]*n_bins for _ in range(len(bkg_models))] for _ in range(len(sig_models))]
            ...
            for i in range(1, hist.GetNbinsX() + 1):
                for j in range(1, hist.GetNbinsY() + 1):
                    ...
                    elif ENABLE_TEST_BIN and (run_no == 2 and run_no2 == 0):
                        chi2s.append(float('inf'))
                        best_comb[0].append(0)
                        best_comb[1].append(0)
                        man_calc.append(0)
