...
def main(
    ...
    #template_cache = {}
    wsp_template = infile.Get("wsp_" + name)
    for bin_index, b in enumerate(bins):
        ...
        #neu:
        wsp = wsp_template.Clone(f"wsp_{best_sig}_{best_bkg}_bin{bin_index}")

        pdf_args, nparams = build_pdf_args(best_sig, best_bkg)
        model_args = build_model_args()

        logger.info(f"Erstelle PDFs für {best_sig}/{best_bkg}:")
        for arg in pdf_args:
            wsp.factory(arg)
        
        logger.info(f"Erstelle Model-Expressionen:")
        for arg in model_args:
            wsp.factory(arg)
        # NEU: Erstelle PDFs für diese spezifische Kombination
        # pdf_args = []
        # nparams = 1
        wsp.saveSnapshot("init_vars", wsp.allVars())
        logger.info(f"Snapshot gespeichert mit {wsp.allVars().getSize()} Variablen")
        ...
        wsp.loadSnapshot("init_vars")

        for v in wsp.allVars():
            if not v.isConstant():
                v.setConstant(False)

        # key = (best_sig, best_bkg)
        # if key not in template_cache:
        #     # Einmalig wie im Finder
        #     wsp_template = infile.Get("wsp_" + name).Clone(f"template_{best_sig}_{best_bkg}") 
        #     pdf_args, nparams = build_pdf_args(best_sig, best_bkg)
        ...
        #     snapshot_name = f"start_params_{best_sig}_{best_bkg}"
        #     wsp_template.saveSnapshot(snapshot_name, params, True)
        #     template_cache[key] = (wsp_template, snapshot_name)
            
        # wsp_template, snapshot_name = template_cache[key]
        # wsp = wsp_template.Clone(f"wsp_{best_sig}_{best_bkg}_bin{bin_index}")
        # wsp.loadSnapshot(snapshot_name)
        ...
        logger.info(f"In bin {label}, yield_tot = {yield_tot}, yield_pass = {yield_pass}, yield_fail = {yield_fail}")
