    ...
    for run_no, sig_model in enumerate(sig_models):
        ...
        for run_no2, bkg_model in enumerate(bkg_models): # remove
            if (bkg_model != "CMSShape") and ENABLE_TEST_BIN:
                continue
            wsp_template = infile.Get("wsp_" + name).Clone(f"wsp_template_{sig_model}_{bkg_model}")
            print(wsp_template)
            ...
            for arg in pdf_args:
                wsp_template.factory(arg)
            for arg in model_args:
                wsp_template.factory(arg)
                                     
            params = wsp_template.allVars()
            snapshot_name = f"start_params_{sig_model}_{bkg_model}"
            wsp_template.saveSnapshot(snapshot_name, params, True)
            ...
            try:
                for i, b in enumerate(bins):
                    ...
                    wsp_bin = wsp_template.Clone(f"wsp_{sig_model}_{bkg_model}_bin{i}")

                    wsp_bin.loadSnapshot(snapshot_name)
                    ...
                    yield_tot = wsp_bin.data(dat).sumEntries()
                    yield_pass = wsp_bin.data(dat).sumEntries("cat==cat::pass")
                    yield_fail = wsp_bin.data(dat).sumEntries("cat==cat::fail")
                    ...
                    wsp_bin.var("numTot").setVal(yield_tot)
                    try:
                        wsp_bin.var("efficiency").setVal(yield_pass / yield_tot)
                        wsp_bin.var("efficiency").setAsymError(0, 0)
                        sds = yield_pass / yield_tot
                    except ZeroDivisionError:
                        wsp_bin.var("efficiency").setVal(0)
                        sds = 0
                    ...
                    nll = wsp_bin.pdf("model").createNLL(wsp_bin.data(dat), ROOT.RooFit.Extended(True), ROOT.RooFit.Offset(True))

                    val, err_lo, err_hi = scan_uncertainty(wsp_bin, "efficiency", nll, wsp_bin.data(dat))

                    eff = wsp_bin.var("efficiency")
                    eff.setVal(val)
                    eff.setError((err_hi + err_lo) / 2.0)
                    wsp_bin.var("efficiency").setError(eff.getError())
                    ...
                    splitData = wsp_bin.data(dat).split(wsp_bin.cat("cat"))
                    xframe = wsp_bin.var("m_vis").frame(ROOT.RooFit.Title("Passing"))
                    width = (wsp_bin.var("m_vis").getMax() - wsp_bin.var("m_vis").getMin()) / splitData.At(
                        1
                    ).numEntries()
                    ...
                    wsp_bin.pdf("passing").plotOn(
                        xframe,
                        ROOT.RooFit.Slice(wsp_bin.cat("cat"), "pass"),
                    ...
                    pads[0].cd()
                    xframe.Draw()
                    effs_sig[run_no][i] = wsp_bin.var("efficiency").getVal()
                    effs_bkg[run_no2][i] = wsp_bin.var("efficiency").getVal()
                    effs[run_no][run_no2][i] = wsp_bin.var("efficiency").getVal()
                    ...
                    if xframe.chiSquare("AllPass", "DataPass", nparams) < chi2s[i]: # Check whether current chi2 is lower than current lowest (starts with inf)
                        ...
                        hist.SetBinContent(b[0], b[1], wsp_bin.var("efficiency").getVal())
                        hist.SetBinError(b[0], b[1], wsp_bin.var("efficiency").getError())

                        hsyst.SetBinContent(b[0], b[1], wsp_bin.var("efficiency").getVal())
                        hsyst2.SetBinContent(b[0], b[1], wsp_bin.var("efficiency").getVal())
                        ...
                        val = wsp_bin.var("efficiency").getVal()
                        err = wsp_bin.var("efficiency").getError()
                        ...
                        xframe2 = wsp_bin.var("m_vis").frame(ROOT.RooFit.Title("Failing"))
                        splitData.At(0).plotOn(
                            xframe2,
                            ROOT.RooFit.DataError(ROOT.RooAbsData.Poisson),
                            ROOT.RooFit.Name("DataFail"),
                        )
                        wsp_bin.pdf("failing").plotOn(
                            xframe2,
                            ROOT.RooFit.Slice(wsp_bin.cat("cat"), "fail"),
                ...
                logger.error(f"Error in bin {i}: {e}")
