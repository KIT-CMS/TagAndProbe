...
def main(
    ...
    # try:
    #     bin_cfgs = apply_percentage_bin_variation_2d(bin_cfgs, percentage=10, variation="down", axis="pteta")
    # except Exception as e:
    #     print(f"Binning_Var_Fehler: {e}")
    #     exit()

    # if only_key:
    #     if only_key not in bin_cfgs:
    #         raise ValueError(f"Key '{only_key}' nicht in {config_filename} gefunden")
    #     bin_cfgs = {only_key: bin_cfgs[only_key]}
    ...
    for key, cfg_dict in bin_cfgs.items():
        ...
        # required_keys = ["tag", "probe", "binvar_x", "binvar_y"]
        # if not all(k in cfg for k in required_keys):
        #     logger.info(f"Skipping block '{key}' (not a TnP config)")
        #     continue

        logger.info(f"== Inspecting block: {key} ==")
        logger.info(f"Keys inside this block: {list(cfg.keys())}")
        logger.info(f"Full content: {cfg}")
    ...
    if channel == "embeddingselection":
    # Nur Data für Embeddingselection verwenden
        trees = {
        ...
        # Normale Kanäle: Data, DY, Embedding
        trees = {
        ...
        # Nur prüfen, ob DY-Datei existiert, wenn DY überhaupt erwartet wird
        dy_path = input_files[era][channel]["DY"]
        if not os.path.exists(dy_path):
            raise FileNotFoundError(f"DY file does not exist: {dy_path}")
