...
def sf_color(process):
    ...
    dict = {"Data": 1, "DY": 98, "Embedding": 60}
    return dict[process]
...
def build_plot(folder, variable, era, type, etabin, plotoptions):
    ...
    for k, process in enumerate(processes):
        rootfile = rootfile_parser.ScaleFactor_Rootfile_parser(
            "{folder}/{type}_TP_{process}_{era}_Fits_{variable}.root".format(
                folder=folder, type=type, era=era, variable=variable, process=process
            )
        )
        ...
        if "Data" in process:
            for label in ["Data_Embedding_ratio", "Data_DY_ratio"]:
                plot.subplot(1).add_hist(rootfile.get(variable, etabin + 1), label)
            ...
            plot.subplot(1).add_hist(
                rootfile.get(variable, etabin + 1), "{}_ratio".format(process)
            )
            plot.subplot(1).setGraphStyle(
    ...
    logger.debug(
        "y Range: {} - {}".format(plotoptions["y_range"][0], plotoptions["y_range"][1])
    )
    logger.debug(
        "y Subrange: {} - {}".format(
            plotoptions["ratio_y_range"][0], plotoptions["ratio_y_range"][1]
        )
    )
    logger.debug(
        "pT Range: {} - {}".format(
            plotoptions["ptrange"][0], int(plotoptions["ptrange"][1])
        )
    )
...
def plot_efficiency(folder, variable, era, type, etabin, plotoptions):
    ...
    # draw subplots. Argument contains names of objects to be drawn in corresponding order.
    plot.subplot(0).Draw(processes)

    plot.add_legend(width=0.3, height=0.15, pos=6)
    for process in processes:
        plot.legend(0).add_entry(0, process, sf_label(type, process), "PLE")
    ...
    logger.debug("Saving Plot")
    plot.save(
        "{folder}/{variable}_{type}_{era}_{etarange}_efficiency.pdf".format(
            folder=plotoptions["outputdir"],
            variable=variable,
            type=type,
            era=era,
    ...
    plot.save(
        "{folder}/{variable}_{type}_{era}_{etarange}_efficiency.png".format(
            folder=plotoptions["outputdir"],
            variable=variable,
            type=type,
            era=era,
