...
def plot_hadronic(input_file, working_points, file_types, era, output, draw_options, title, y_range, ratio_y_range, binned_in, x_title, ratio_to, plot_dir, label_pos):
    ...
    for wp in working_points:
        ...
        for ft in file_types:
                hists[wp].append(file.Get("hist_MuTauTriggerEfficiency_{}TauMVA_{}".format(wp, ft)).Clone())
    file.Close()
    ...
    for wp in working_points:
        ...
        #~ if 'ID' in splitsrc[1]:
            #~ legend = ROOT.TLegend(0.18, 0.67, 0.5, 0.85, '', 'NDC')		
        for j, hist in enumerate(hists[wp]):
            #splitsrc = src.split(':')
            # htgr = hists[j].ProjectionX('%s_projx_%i' % (hists[j].GetName(), j), i, i)
            #if len(splitsrc) >= 3:
            #    settings = {x.split('=')[0]: eval(x.split('=')[1]) for x in splitsrc[2].split(',')}
