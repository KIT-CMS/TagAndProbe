...
def plot_hadronic(input_file, triggers, working_points, file_types, era, per_dm, output, draw_options, title, y_range, ratio_y_range, binned_in, x_title, ratio_to, plot_dir, label_pos):
    ...
    for trg in triggers:
        for decayMode in decayModes:
            ...
            for wp in working_points:
                ...
                #~ plot.DrawCMSLogo(pads[0], args.title, bin_label, 0, 0.16, 0.035, 1.2, cmsTextSize=0.5)
                plot.DrawTitle(pads[0], trg+' '+title+dm_label.replace('_','').replace('dm',' - dm')+' - '+bin_label, 1)
