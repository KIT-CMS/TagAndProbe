...
class TauLegEfficiencies(object):
    ...
    def add_wp(self, wp):
        if wp not in self._config["tauId_wps"].keys():
            logger.fatal(
                "Given working point %s is not available."
                " MVA working point must be one of %s",
                wp,
                self._config["tauId_wps"].keys(),
            )
            raise Exception
    ...
    def add_trigger_name(self, trigger):
        available_trgs = set(
            key for val in self._config["trigger_dict"].values() for key in val.keys()
        )
        logger.debug("Set of possible trigger paths: %s", available_trgs)
        if trigger not in available_trgs:
            logger.fatal(
                "Given trigger path %s is not available."
    ...
    def _create_file_regex(self, filetype, use_et=False):
        ...
        glob_exp = "{inp}{fi}/{fi}.root".format(
            inp=self.input_dir, fi=file_dict[filetype]
        )
        logger.debug("Looking for files matching to glob expression: %s", glob_exp)
        ...
        for filetype in self.filetypes:
            ...
            logger.debug(
                "Cutstring: {}".format(
                    "("
                    + ")&&(".join(
                        self._config["baseline_selection"]
                        if filetype == "DATA"
            ...
            if self._use_et:
                ...
                logger.debug(
                    "Cutstring: {}".format(
                        "("
                        + ")&&(".join(
                            self._config["baseline_selection_et"]
                            if filetype == "DATA"
                ...
                logger.info(
                    "Writing resulting histograms to %s.", self._outfile.GetName()
                )
                for eff in self._efficiencies:
                    eff.save_histograms()
                self._efficiencies = []
...
class Efficiency(object):
    ...
    def __init__(
        ...
        self._categories = yaml.load(open("settings/config_tau_legs.yaml", "r"))[
            self._era
        ]["etaphi_cats"]
    ...
    def save_histograms(self):
        logger.info(
            "Saving %s histograms for tau leg of the %s trigger and"
            " %s MVA working point",
            self.suffix,
            self.trigger_name,
            self.working_point,
    ...
    def _create_histogram_models(self):
        bin_settings = yaml.load(open("settings/settings_tau_trigger.yaml", "r"))
        if self._decayMode is None:
            bins = bin_settings["bins_fits"][self._era][self.trigger_name]
        else:
            bins = bin_settings["bins_fits_dm"][self._era][self.trigger_name][
                self.decayMode
            ][self.suffix]
        self._mod_th1d_pT = root.RDF.TH1DModel(
            ...
            "mod_th1d_pT",
            len(bins["pT"]) - 1,
            np.array(bins["pT"], dtype=float),
        )
        self._mod_th2d_eta_phi = root.RDF.TH2DModel(
            "mod_th2d_eta_phi",
            "mod_th2d_eta_phi",
            len(bins["eta"]) - 1,
            np.array(bins["eta"], dtype=float),
            len(bins["phi"]) - 1,
            np.array(bins["phi"], dtype=float),
        )
        self._mod_th2d_eta_phi_AVG = root.RDF.TH2DModel(
            "mod_th2d_eta_phi_AVG",
            "mod_th2d_eta_phi_AVG",
            len(bins["etaAVG"]) - 1,
            np.array(bins["etaAVG"], dtype=float),
            len(bins["phiAVG"]) - 1,
            np.array(bins["phiAVG"], dtype=float),
        )
        return

    def _create_histogram_pointers(self, dataframe, trigger_dict):
        logger.debug(
            "Creating %s histograms for tau leg of the %s trigger"
        ...
        self.hist1d_total = dataframe.Histo1D(self._mod_th1d_pT, "pt_p", "bkgSubWeight")
        ...
        pt_cut = yaml.load(open("settings/settings_tau_trigger.yaml", "r"))[
            "pt_cuts_2d"
        ][self._era][self.trigger_name]
        ...
        for cat, cutstr in self._categories.iteritems():
            ...
            self._hist2d_pointers[cat]["total"] = dataframe_2d.Filter(
                "(" + ")&&(".join(cutstr) + ")"
            ).Histo2D(self._mod_th2d_eta_phi, "eta_p", "phi_p", "bkgSubWeight")
            self._hist2d_pointers[cat]["pass"] = (
                dataframe_2d.Filter("(" + ")&&(".join(cutstr) + ")")
    ...
    def _save_1Dhistogram(self):
        ...
        g_eff = root.TGraphAsymmErrors(
            self.hist1d_pass.GetValue(), self.hist1d_total.GetValue()
        )

        h_fail = root.TH1D(self.hist1d_total.GetValue())
        h_fail.Sumw2()
        h_fail.Add(self.hist1d_pass.GetValue(), -1.0)
        g_eff_v2 = root.RooHist(
            self.hist1d_pass.GetValue(),
            h_fail,
            0,
            1.0,
            root.RooAbsData.Poisson,
            1.0,
            root.kTRUE,
            1.0,
        )
    ...
    def _save_2Dhistograms(self):
        for cat, hist_ptrs in self._hist2d_pointers.iteritems():
            ...
            h_eff.Sumw2()
            h_eff.Divide(
                hist_ptrs["pass"].GetValue(),
                hist_ptrs["total"].GetValue(),
                1.0,
                1.0,
                "b(1,1) cl=0.683 mode",
            )
