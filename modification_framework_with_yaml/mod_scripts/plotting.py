...
def makeHist1D(name, xbins, graph, scaleXrange=1.0):
    ...
    hist = R.TH1F(
        ...
        xbins,
        graph.GetX()[0],
        scaleXrange * (graph.GetX()[graph.GetN() - 1] + binw_x),
    )
    return hist
...
def FixBoxPadding(pad, box, frac):
    ...
    f_x1 = (p_x1 - pad.GetLeftMargin()) / (
        1.0 - pad.GetLeftMargin() - pad.GetRightMargin()
    )
    f_x2 = (p_x2 - pad.GetLeftMargin()) / (
        1.0 - pad.GetLeftMargin() - pad.GetRightMargin()
    )
    f_y1 = (p_y1 - pad.GetBottomMargin()) / (
        1.0 - pad.GetTopMargin() - pad.GetBottomMargin()
    )
    ...
    if R.gPad.GetLogy() and f_max_h > 0.0:
        f_max_h = (math.log10(a_max_h) - math.log10(ymin)) / (
...
def PositionedLegend(width, height, pos, offset, horizontaloffset=None):
    ...
    if pos == 3:
        return R.TLegend(
            1 - r - ho - w, 1 - t - o - h, 1 - r - ho, 1 - t - o, "", "NBNDC"
        )
    if pos == 4:
